...
{
    /// <summary>
    /// Logic for the healer role interactions with the combat routine.
    /// </summary>
    /// <inheritdoc cref="M:Komabatant.Interfaces.LogicExecutor"/>
    internal class Healer : LogicExecutor
    {
        #region Singleton

        private static Healer _healer;
        internal static Healer Instance => _healer ?? (_healer = new Healer());

        #endregion

        /// <summary>
        /// Main task executor for the healer logic.
        /// </summary>
        ...
        internal new async Task<bool> ExecuteLogic()
        {
            ...
            if(!Core.Me.IsHealer() || !Core.Me.CanFight())
                return false;
